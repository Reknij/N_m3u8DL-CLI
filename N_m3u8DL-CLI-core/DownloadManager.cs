    ...
    class DownloadManager
    {
        ...
        public void DoDownload()
        {
            ...
            try
            {
                ...
                LOGGER.WriteLine(stringscore.hasExternalAudioTrack);
                LOGGER.PrintLine(stringscore.hasExternalAudioTrack, LOGGER.Warning);
            }
            catch (Exception) {}
            try
            {
                ...
                LOGGER.WriteLine(stringscore.hasExternalSubtitleTrack);
                LOGGER.PrintLine(stringscore.hasExternalSubtitleTrack, LOGGER.Warning);
            }
            catch (Exception) { }
            ...
            //开始调用下载
            LOGGER.WriteLine(stringscore.startDownloading);
            LOGGER.PrintLine(stringscore.startDownloading, LOGGER.Warning);

            //下载MAP文件（若有）
            downloadMap:
            if (HasExtMap)
            {
                LOGGER.PrintLine(stringscore.downloadingMapFile);
            ...
            if (!File.Exists(Path.Combine(DownDir, "Part_" + 0.ToString(partsPadZero), firstSeg["index"].Value<int>().ToString(segsPadZero) + ".ts")))
            {
                try
                {
                    ...
                    if (File.Exists(sd.SavePath))
                        File.Delete(sd.SavePath);
                    LOGGER.PrintLine(stringscore.downloadingFirstSegement);
            ...
            if (Global.HadReadInfo == false)
            {
                ...
                LOGGER.PrintLine(stringscore.waitForCompletion, LOGGER.Warning);
                if (!flag)
        ...
        ll:
            if (tsCount != segCount)
            {
                LOGGER.PrintLine(stringscore.downloadedCount + tsCount + " / " + segCount);
                LOGGER.WriteLine(stringscore.downloadedCount + tsCount + " of " + segCount);
                if (Count <= RetryCount)
                {
                    Count++;
                    LOGGER.WriteLine(stringscore.retryCount + Count + " / " + RetryCount);
                    LOGGER.PrintLine(stringscore.retryCount + Count + " / " + RetryCount, LOGGER.Warning);
                    Thread.Sleep(3000);
                    DoDownload();
                }
            }
            ...
            {
                LOGGER.PrintLine(stringscore.downloadComplete + (DisableIntegrityCheck ? "(" + stringscore.disableIntegrityCheck + ")" : ""));
                if (NoMerge == false)
                {
                    string exePath = Path.GetDirectoryName(Process.GetCurrentProcess().MainModule.FileName);
                    //string driverName = exePath.Remove(exePath.IndexOf(':'));
                    Console.Title = "Done.";
                    LOGGER.WriteLine(stringscore.startMerging);
                    LOGGER.PrintLine(stringscore.startMerging, LOGGER.Warning);
                    //VTT字幕
                    if (isVTT == true)
                    {
                        MuxFormat = "vtt";
                    ...
                    if (PartsCount == 1)
                    {
                        ...
                        if (BinaryMerge)
                        {
                            LOGGER.PrintLine(stringscore.binaryMergingPleaseWait);
                            MuxFormat = "ts";
                        ...
                        else
                        {
                            if (Global.VIDEO_TYPE != "DV") //不是杜比视界
                            {
                                ...
                                foreach (string s in Global.GetFiles(Path.Combine(DownDir, "Part_0"), ".ts"))
                                {
                                    ...
                                    {
                                        //转换
                                        LOGGER.PrintLine(stringscore.remuxToMPEGTS + Path.GetFileName(s));
                                        LOGGER.WriteLine(stringscore.remuxToMPEGTS + Path.GetFileName(s));
                                ...
                                if (tsCount >= 1800)
                                {
                                    LOGGER.WriteLine(stringscore.partialMergingPleaseWait);
                                    LOGGER.PrintLine(stringscore.partialMergingPleaseWait, LOGGER.Warning);
                                ...
                                LOGGER.PrintLine(stringscore.ffmpegMergingPleaseWait);
                        ...
                        if (externalAudio)  //下载独立音轨
                        {
                            ...
                            DownloadedSize = 0;
                            Global.WriteInit();
                            LOGGER.PrintLine(stringscore.downloadingExternalAudioTrack, LOGGER.Warning);
                            ...
                            LOGGER.WriteLine(stringscore.startParsing + externalAudioUrl);
                            LOGGER.WriteLine(stringscore.downloadingExternalAudioTrack);
                        ...
                        if (externalSub)  //下载独立字幕
                        {
                            ...
                            DownloadedSize = 0;
                            Global.WriteInit();
                            LOGGER.PrintLine(stringscore.downloadingExternalSubtitleTrack, LOGGER.Warning);
                            Parser parser = new Parser();
                            parser.Headers = Headers; //继承Header
                            parser.BaseUrl = "";
                            ...
                            parser.DownDir = Path.Combine(Path.GetDirectoryName(DownDir), parser.DownName);
                            LOGGER.WriteLine(stringscore.startParsing + externalSubUrl);
                            LOGGER.WriteLine(stringscore.downloadingExternalSubtitleTrack);
                        ...
                        LOGGER.PrintLine(stringscore.taskDone, LOGGER.Warning);
                    ...
                    //合并分段
                    LOGGER.PrintLine(stringscore.startMerging);
                    for (int i = 0; i < PartsCount; i++)
                    {
